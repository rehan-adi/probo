...
const logAudit = async (action: string, userId?: string, ip?: string, userAgent?: string, metadata?: any) => {
	try {
...
export const checkUsername = async (c: Context) => {
	try {
		...
		return c.json({
			success: true, data: { isAvailable: !existingUser }
		});
	} catch (error) {
...
export const updateUsername = async (c: Context) => {
	try {
		...
		const existingUser = await prisma.user.findFirst({
			where: {
				username: {
					equals: result.data.username, mode: 'insensitive'
				}
			},
		});
		...
		const updatedUser = await prisma.user.update({
			where: { id: user.id },
			data: {
				username: result.data.username, avatarUrl, onboardingStatus: 'PENDING_PREFERENCES'
			},
		});

		await logAudit('SET_USERNAME', user.id, undefined, undefined, { username: result.data.username });
	...
	try {
		...
		if (body.referralCode) {
			...
			if (refResult.success && refResult.data.referralCode) {
				try {
					const referrer = await prisma.user.findUnique({ where: { referralCode: refResult.data.referralCode } });
					if (!referrer) return c.json({ success: false, error: 'Invalid referral code' }, 400);
					if (referrer.id === user.id) return c.json({ success: false, error: 'Cannot use your own referral code' }, 400);

					const dbUser = await prisma.user.findUnique({ where: { id: user.id } });
					if (dbUser?.referrerId) return c.json({ success: false, error: 'Referral code already applied' }, 400);

					await prisma.$transaction(async (tx) => {
						await tx.user.update({ where: { id: user.id }, data: { referrerId: referrer.id } });
						await tx.referral.create({ data: { referrerId: referrer.id, referredId: user.id, amount: 15.0, isReferrer: true, status: 'PENDING' } });
						await tx.referral.create({ data: { referrerId: referrer.id, referredId: user.id, amount: 10.0, isReferrer: false, status: 'PENDING' } });
					});
					await logAudit('APPLY_REFERRAL', user.id, undefined, undefined, { referralCode: refResult.data.referralCode });
				} catch (err: any) {
		...
		if (!prefsResult.success) return c.json({ success: false, error: prefsResult.error.issues }, 400);
