			...
			return (
				<div className="bg-white dark:bg-[#2C2C2E] border border-gray-200 dark:border-white/10 p-3 rounded-lg shadow-xl">
					...
					{payload.map((entry: any, index: number) => (
						<p key={index} className="text-sm font-bold text-gray-900 dark:text-white flex items-center">
							<span className="mr-2" style={{ color: entry.color || entry.fill }}>●</span>
			...
			<div className="space-y-6">
				...
				<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
					<Card className="dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200 overflow-hidden relative">
						...
						<CardContent>
							<div className="text-2xl font-bold text-gray-900 dark:text-white">{formatINR(metrics?.totalRevenue)}</div>
					...
					<Card className="dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200 overflow-hidden relative">
						...
						<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
							<CardTitle className="text-sm font-medium text-gray-600 dark:text-gray-400">Active Markets</CardTitle>
							<div className="w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center">
								<Activity className="h-4 w-4 text-blue-600 dark:text-blue-400" />
							</div>
						</CardHeader>
						<CardContent>
							<div className="text-2xl font-bold text-gray-900 dark:text-white">{formatNumber(metrics?.totalMarkets)}</div>
							<div className="flex items-center mt-1 space-x-2">
								<span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded">
									+4
								</span>
					...
					<Card className="dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200 overflow-hidden relative">
						...
						<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
							<CardTitle className="text-sm font-medium text-gray-600 dark:text-gray-400">Trading Volume</CardTitle>
							<div className="w-8 h-8 rounded-full bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center">
								<TrendingUp className="h-4 w-4 text-amber-600 dark:text-amber-400" />
							</div>
						</CardHeader>
						<CardContent>
							<div className="text-2xl font-bold text-gray-900 dark:text-white">{formatINR(metrics?.totalVolume)}</div>
							<div className="flex items-center mt-1 space-x-2">
								<span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded">
									+24.8%
								</span>
					...
					<Card className="dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200 overflow-hidden relative">
						...
						<CardContent>
							<div className="text-2xl font-bold text-gray-900 dark:text-white">{formatNumber(metrics?.totalUsers)}</div>
							<div className="flex items-center mt-1 space-x-2">
								<span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded">
									+182
								</span>
				...
				<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2 mt-6">
					<Card className="col-span-1 dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200">
						<CardHeader className="pb-2">
							...
							<CardDescription className="text-gray-500 dark:text-gray-400">Daily revenue over the {period === '7d' ? 'last 7 days' : period === '30d' ? 'last 30 days' : 'last 90 days'}.</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="h-[300px] w-full mt-4">
								{metrics?.revenueChart && metrics.revenueChart.length > 0 ? (
									<ResponsiveContainer width="100%" height="100%">
										<AreaChart data={metrics.revenueChart} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
											<defs>
					...
					<Card className="col-span-1 dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200">
						<CardHeader className="pb-2">
							<CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">Trading Volume Overview</CardTitle>
							<CardDescription className="text-gray-500 dark:text-gray-400">Daily trading volume over the {period === '7d' ? 'last 7 days' : period === '30d' ? 'last 30 days' : 'last 90 days'}.</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="h-[300px] w-full mt-4">
								{metrics?.revenueChart && metrics.revenueChart.length > 0 ? (
				...
				<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7 mt-6">
					<Card className="col-span-4 lg:col-span-7 dark:bg-[#1C1C1E] dark:border-white/10 shadow-sm border-gray-200">
						<CardHeader className="pb-2">
							<CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">Market Status</CardTitle>
							<CardDescription className="text-gray-500 dark:text-gray-400">Distribution of all markets.</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="h-[300px] w-full mt-8">
								{metrics?.marketDistribution && metrics.marketDistribution.some((m: any) => m.value > 0) ? (
									<ResponsiveContainer width="100%" height="100%">
										<PieChart>
											<Pie
											...
											<Tooltip 
												contentStyle={{ borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', backgroundColor: 'var(--tw-prose-body, #2C2C2E)' }}
