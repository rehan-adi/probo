...
export default function Settings() {
	...
	const [emailOrderFills, setEmailOrderFills] = useState(false);
		...
		const fetchSettings = async () => {
			try {
				...
				if (res.data.notificationPrefs) {
					...
					setEmailOrderFills(prefs.emailOrderFills);
				...
				<div className="flex-1 max-w-3xl">
					{activeTab === 'profile' && (
						<div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
							...
							{/* Bio Section */}
							<div>
								<h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3 px-1">Bio</h3>
							...
							{bio !== user?.bio && (
								<div className="flex justify-end">
									<button
										...
										disabled={loading}
										className="bg-black dark:bg-white text-white dark:text-black px-6 py-2 rounded-lg font-medium text-sm hover:opacity-90 disabled:opacity-50"
									>
							...
							<div className="bg-white dark:bg-gray-900 border border-red-100 dark:border-red-900/30 rounded-2xl p-6">
								<div className="flex items-start justify-between gap-8">
									...
									<button
										...
										className="shrink-0 flex items-center gap-2 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-500 px-4 py-2 rounded-lg font-medium text-sm hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors"
									>
