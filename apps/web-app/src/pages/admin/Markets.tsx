...
import { Loader2, CheckCircle, XCircle, Ban, Search, Activity, CalendarDays, Users, AlertTriangle, X } from 'lucide-react';
			...
			<div style={{
				...
				<div className="flex items-start justify-between p-5 border-b border-gray-200 dark:border-white/10">
					<div className="flex-1 min-w-0 pr-3">
						...
						<p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2">{market.title}</p>
					</div>
					<button
						type="button"
						disabled={busy}
				...
				<div className="p-5 flex flex-col gap-2">
					...
					>
						{busy ? <Loader2 style={{ width: 16, height: 16 }} className="animate-spin shrink-0" /> : <CheckCircle style={{ width: 16, height: 16, flexShrink: 0 }} />}
						Resolve as YES
					</button>
					...
					>
						{busy ? <Loader2 style={{ width: 16, height: 16 }} className="animate-spin shrink-0" /> : <XCircle style={{ width: 16, height: 16, flexShrink: 0 }} />}
						Resolve as NO
					</button>
					...
					>
						{busy ? <Loader2 style={{ width: 16, height: 16 }} className="animate-spin shrink-0" /> : <Ban style={{ width: 16, height: 16, flexShrink: 0 }} />}
...
export default function AdminMarkets() {
	...
	const handleResolve = useCallback(async (marketId: string, result: 'YES' | 'NO' | 'CANCEL') => {
		...
		try {
			...
			toast.error(err.response?.data?.error || err.response?.data?.message || err.message || 'Error resolving market');
		} finally {
			setResolvingId(null);
		}
	}, [fetchMarkets]);
	...
	const filtered = markets.filter(m => {
		...
		const matchSearch = !search || m.title?.toLowerCase().includes(search.toLowerCase()) || m.symbol?.toLowerCase().includes(search.toLowerCase());
	...
	return (
		<AdminLayout>
			<div className="space-y-6">
				{/* Header */}
				<div>
					<h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Markets</h2>
					<p className="text-gray-500 dark:text-gray-400 mt-1">Manage and resolve markets across the platform.</p>
				</div>
							...
							<table className="w-full text-sm">
								<thead className="bg-gray-50 dark:bg-[#2C2C2E]">
									<tr className="border-b border-gray-200 dark:border-white/10">
										<th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Market</th>
										<th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Status</th>
										<th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Volume</th>
										<th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Traders</th>
										<th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ends At</th>
								...
								<tbody>
									{paged.length === 0 ? (
										...
										<tr key={market.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors">
											{/* Title */}
											<td className="px-4 py-3">
												<div className="flex items-center gap-3 max-w-[200px]">
													{market.thumbnail ? (
														<img src={market.thumbnail} alt="" className="w-9 h-9 rounded-md object-cover border border-gray-200 dark:border-white/10 shrink-0" />
													) : (
													...
													<div className="min-w-0">
														<p className="font-medium text-gray-900 dark:text-white truncate" title={market.title}>{market.title}</p>
														<p className="text-xs text-gray-400 truncate">{market.symbol || market.id?.substring(0, 8)}</p>
													</div>
												</div>
											</td>
				...
				{!loading && totalPages > 1 && (
					<div className="flex items-center justify-between">
						<span className="text-sm text-gray-500 dark:text-gray-400">
							Showing {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, filtered.length)} of {filtered.length} markets
						</span>
			...
			{resolveTarget && (
				<ResolveModal
					...
					onClose={() => { if (!resolvingId) setResolveTarget(null); }}
				/>
			)}
		</AdminLayout>
	);
