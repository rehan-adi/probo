...
export default function Search() {
	...
	return (
		<div className="pt-24 pb-12 px-4 max-w-5xl mx-auto min-h-screen text-foreground">
			...
			{query.trim() && <div className="mb-6 text-gray-500">Found {total} results</div>}
			...
			<div className="flex flex-col gap-4">
				...
				{isLoading && (
					<div className="flex flex-col gap-4">
						{[...Array(3)].map((_, i) => (
							<div
								key={i}
								className="flex gap-4 p-4 bg-card border border-border rounded-xl animate-pulse"
							>
				...
				{!isLoading && results.length === 0 && query.trim() !== '' && (
					<div className="text-center text-gray-500 mt-12">No markets found for "{query}".</div>
				)}
			</div>
		</div>
	);
