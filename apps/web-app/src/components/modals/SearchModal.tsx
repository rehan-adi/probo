	...
	return (
		<div className="fixed inset-0 z-[100] flex flex-col justify-end bg-black/40 backdrop-blur-sm md:hidden" onClick={onClose}>
			<motion.div
				...
				className="w-full h-[85vh] bg-white dark:bg-[#090C1A] rounded-t-2xl shadow-xl flex flex-col overflow-hidden"
				...
				<div className="px-4 py-2 border-b border-gray-100 dark:border-gray-800 flex items-center gap-3">
					<div className="relative flex-1">
						...
						<input
							...
							className="w-full bg-[#F4F5F6] dark:bg-slate-800 text-gray-900 dark:text-gray-100 rounded-lg pl-10 pr-10 py-3 text-[15px] focus:outline-none focus:bg-white dark:focus:bg-slate-900 focus:ring-1 focus:ring-gray-200 dark:focus:ring-gray-700 transition-all placeholder:text-gray-500"
							value={query}
						...
						{query && (
							<button
								onClick={() => setQuery('')}
								className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full text-gray-500 transition-colors"
							>
								<X className="w-4 h-4" />
							</button>
						)}
							...
							{isLoading ? (
								...
								results.map((market) => (
									<div
										...
										className="flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-slate-800/50 rounded-lg cursor-pointer transition-colors"
									>
										<img
								...
								<div className="grid grid-cols-2 gap-2">
									{BROWSE_CATEGORIES.map((category) => (
										<button
											...
											className="flex items-center gap-2.5 p-3 hover:bg-gray-50 dark:hover:bg-slate-800 rounded-xl text-gray-700 dark:text-gray-300 font-medium text-sm transition-colors border border-transparent hover:border-gray-100 dark:hover:border-slate-700"
										>
